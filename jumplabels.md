...
A normal `if (flag)` still loads that flag from memory on every hit.
Jump labels — the mechanism behind the static-key API — rewrite the
machine code instead. While the feature is off, the hot path is a `nop`
(or an unconditional `jmp`). Turning the
feature on patches `nop` <=> `jmp` in place. Toggling is expensive;
running the common path is nearly free.

The first part of the document, that goes from section [](#the-problem-jump-labels-solve){.secref}
...
`.text` plus an entry in [`__jump_table`](https://elixir.bootlin.com/linux/v7.2/source/scripts/module.lds.S#L31). **"Static key"** is the
higher-level API (`DEFINE_STATIC_KEY_*`, `static_branch_*`). This tutorial
keeps them distinct.
...
[`jump_label_init()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L525) runs, so it cannot read the live
[`enabled`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L87) count. It determines the code generated at compile time:

1. **Key initial value** — `DEFINE_STATIC_KEY_TRUE` vs `_FALSE`.
2. **Call-site hint** — [`static_branch_likely()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L474) vs [`static_branch_unlikely()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L486).

When those agree, the compiled-in instruction is `nop`. When they disagree,
it is `jmp`. Get them out of sync with the steady state of the key — the
mistake [](#choosing-true-vs-false-and-likely-vs-unlikely){.secref} warns about — and the hot path starts as a `jmp` and
stays that way until someone toggles the key.

The comment at [`include/linux/jump_label.h`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L31) draws the matrix:
...
The runtime patcher uses the same rule, with the live count in place of the
declared default. [`jump_label_type()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L455) is that rule;
[`jump_label_init_type()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L603) is identical except it reads the
compile-time `type` bit of the key. `branch` is 1 for `likely`, 0 for
`unlikely`. At compile time the same XOR is `type ^ branch`. The function
below is the live half:
...
[`static_branch_likely()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L474) and [`static_branch_unlikely()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L486) never
touch a patchable site directly. Underneath them sit two arch-specific
helpers:

- [`arch_static_branch`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/jump_label.h#L35) is the **nop-by-default** helper: the site it
  patches starts out as a plain `nop` (or, thanks to the compile-time hack in
  [](#have-jump-label-hack-why-sites-are-2-or-5-bytes){.secref}, a `jmp` that `objtool` has already rewritten into a same-sized `nop`
  before the kernel even boots).
- [`arch_static_branch_jump`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/jump_label.h#L45) is the opposite: the **jmp-by-default** helper,
...
[](#the-two-asm-helpers){.secref} shows the `asm goto` behind that.

The return value is which instruction is at the site *right now*. Whether
that means "the if-body runs" is the `!` fix-up after the table below.
...
mean "does the if-body run?" for either helper. The helpers themselves
...
The `true`/`false` argument is also stored in the jump-table entry for this
site ([](#the-jump-table-entry-sidecar-metadata){.secref}). The runtime patcher rereads it whenever the key
toggles.

[`__builtin_types_compatible_p`](https://gcc.gnu.org/onlinedocs/gcc/Other-Builtins.html) distinguishes [`struct static_key_true`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L351) vs
[`struct static_key_false`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L355) at compile time; anything else calls
[`____wrong_branch_error()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L405) (unresolved symbol → link error).

### The two asm helpers {#the-two-asm-helpers}

[](#how-the-macros-pick-the-asm){.secref} treated [`arch_static_branch()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/jump_label.h#L35)/[`arch_static_branch_jump()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/jump_label.h#L45) as black
boxes that return `false` or `true`; this is what is actually inside them,
from [`arch/x86/include/asm/jump_label.h`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/jump_label.h):
...
- **Numeric local labels**, like the bare `1:` seen below, are disposable,
  ...
  They exist so that a macro expanded many times over (as [`JUMP_TABLE_ENTRY`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/jump_label.h#L15)
...
never both, depending on [`HAVE_JUMP_LABEL_HACK`](https://elixir.bootlin.com/linux/v7.2/source/arch/Kconfig#L1399) ([](#have-jump-label-hack-why-sites-are-2-or-5-bytes){.secref} explains what decides
...
- **With the hack** (the normal case on x86_64): line for line, this is
  ...
  find and act on before the kernel ever boots ([](#have-jump-label-hack-why-sites-are-2-or-5-bytes){.secref} and [](#assembly-level-picture){.secref} explain that
  signal in full).
...
- `_ASM_ALIGN` expands to a plain `.balign 8` on x86_64 (`.balign 4` on
  ...
  avoiding.
- [`ANNOTATE_DATA_SPECIAL`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/annotate.h#L112) is itself a small assembler macro (its own
...
- `_ASM_PTR " " key " - ."` is the third field, with one difference:
  ...
  `"%c0 + %c1 + 2"`, [](#have-jump-label-hack-why-sites-are-2-or-5-bytes){.secref} explains the `+ 2`). Once the compiler substitutes
  ...
  **ORs the branch hint into bit 0** of the stored value — the same bit
  [`jump_entry_is_branch()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L153) reads back later.
...
with 2 low bits reserved as flags — see [](#struct-jump-entry-relative-form){.secref}) are computed the same way,
each from *its own* field address rather than that of the struct:
[`jump_entry_target()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L122) is `&entry->target + entry->target`, and
[`jump_entry_key()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L127) is the same shape with the flag bits masked off first.
...
This is also precisely what makes the table survive KASLR ([](#struct-jump-entry-relative-form){.secref}) with zero
...
[`jump_label_init()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L525) to go fix up on this front. The encoding is already
...
[](#the-two-asm-helpers){.secref} showed the nop-default site of [`arch_static_branch`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/jump_label.h#L35) written as a hand-coded
...
3. **objtool** ([`tools/objtool/check.c:handle_jump_alt`](https://elixir.bootlin.com/linux/v7.2/source/tools/objtool/check.c#L1872)), which runs once
   ...
   as an objtool-only signal, [`jump_label_init()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L525) later repurposes that
   same bit position for something unrelated — the "`__init` text" flag,
   via [`jump_entry_set_init()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L163) — since nothing at runtime ever needs the
...
The catch is that [`struct jump_entry`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L111) ([](#the-jump-table-entry-sidecar-metadata){.secref}) never records which of the two
...
[`static_branch_enable()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L522)/[`static_branch_disable()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L523) — it cannot just trust a
...
Bit 1 of that key field — the same low-order bits [`jump_entry_key()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L127)
masks off before turning it into a [`struct static_key`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L86) pointer ([](#the-jump-table-entry-sidecar-metadata){.secref}) —
...
is repurposed as what [`jump_entry_is_init()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L158) reads back as the `__init`-text
flag ([](#boot-jump-label-init){.secref}). Conflating the two is an easy way to misread this code cold:
...
- `+2` is not a fourth polarity option alongside `key`/`branch` — it is
  purely the build-time-only "objtool, please NOP this" signal from [](#have-jump-label-hack-why-sites-are-2-or-5-bytes){.secref},
  living in bit 1 of the same field that carries the branch hint in bit 0.
  It means nothing by the time the kernel is running; [`jump_label_init()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L525)
...
Everything in this section is driven by one [`atomic_t`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/types.h#L188): [`key->enabled`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L87). Its
...
- **`-1`** — transient: an enable is in progress, [`jump_label_update()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L886) is
  ...
  [`static_key_count()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L104) deliberately reports `-1` as "1" (enabled) to close
  that gap.
...
[`static_branch_inc()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L513)/[`static_branch_dec()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L514) ([](#boolean-enable-vs-refcounted-enable){.secref}) climb and descend it *without ever
...
[`static_branch_enable()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L522) or [`static_key_slow_inc()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L186) ([](#boolean-enable-vs-refcounted-enable){.secref}, [](#enabling-static-key-enable-static-branch-enable){.secref}) would have
nothing to walk if called this early.

[`arch/x86/xen/multicalls.c`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/xen/multicalls.c) declares
[`static struct static_key mc_debug __ro_after_init;`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/xen/multicalls.c#L60) and registers
[`xen_mc_debug`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/xen/multicalls.c#L78) as an [`early_param()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/init.h#L354)
...
1. Optionally run [`arch_jump_label_transform_static`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L402) for NOP sites — **a
   no-op on x86**, since `objtool` and the compiler have already left the
   right bytes in place ([](#have-jump-label-hack-why-sites-are-2-or-5-bytes){.secref}); other architectures without that build-time
...
[`static_key_seal()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L567) keeps only the [`JUMP_TYPE_TRUE`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L195) bit of the
current [`key->type`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L96) and throws the rest of the word away, replacing it with
[`JUMP_TYPE_LINKED`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L196) plus that one preserved bit. Recall from [](#struct-static-key){.secref} that this
...
never toggled again after boot: once sealed, [`key->entries`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L97) is gone, so
...
sealed key, [`jump_label_add_module()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L690) skips allocating and chaining a
[`struct static_key_mod`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L613) for it — the bookkeeping [](#modules-the-trickiest-part){.secref} otherwise needs so a
...
The ordering against [`mark_rodata_ro()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/mm/init_64.c#L1405) follows
from the same fact: sealing is the last write anything makes to
[`key->type`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L96), and that field sits inside the very `__ro_after_init` section
...
[`drivers/md/dm-stats.c`](https://elixir.bootlin.com/linux/v7.2/source/drivers/md/dm-stats.c#L419) shows a concrete case of this ([](#reading-the-state-without-taking-the-branch){.secref}): the first
time a user asks the device-mapper stats ioctl to start recording
per-region I/O counters, it flips [`stats_enabled`](https://elixir.bootlin.com/linux/v7.2/source/drivers/md/dm.c#L76), a key that has sat
disabled since boot:
...
[`static_branch_enable(&stats_enabled)`](https://elixir.bootlin.com/linux/v7.2/source/drivers/md/dm-stats.c#L420) does run, and from that call onward
...
[`static_key_enable_cpuslocked()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L197) below is what makes that safe:
...
2. [`jump_label_mutex`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L23) serializes all jump-label patching globally, so two
...
3. It sets `enabled = -1` before touching any code — this is the `0 → -1`
   ...
   [`jump_label_update()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L886) below safe to run concurrently with readers:
...
[`static_key_fast_inc_not_disabled()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L127) is what makes "further incs" in the table
...
[](#enabling-static-key-enable-static-branch-enable){.secref} walked through what happens when a key crosses from off to on.
...
[`jump_label_update()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L886) only has to run on the one transition that actually
...
[`static_key_fast_inc_not_disabled()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L127) from [](#enabling-static-key-enable-static-branch-enable){.secref} on the enable side — a bare CAS
...
[`static_key_enabled()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L407) during that window is simply told "off" a
...
[`static_key_dec_not_one()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L253), which performs a plain atomic decrement
...
another CPU wins the race and changes [`key->enabled`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L87) between the read of
...
[`__static_key_slow_dec_deferred()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L346) opens with the exact same
[`static_key_dec_not_one()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L253) check [](#disabling){.secref} just introduced: if this decrement
...
*would* disable the key. Where the [`__static_key_slow_dec_cpuslocked()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L292) from [](#disabling){.secref}
reaches straight for [`jump_label_update()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L886) at that point, this function
instead calls [`schedule_delayed_work()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/workqueue.h#L853) — a standard kernel workqueue
...
Every path in [](#enabling-static-key-enable-static-branch-enable){.secref} through [](#deferred-rate-limited-dec){.secref} eventually funnels into this function — it
...
[`jump_entry_key(entry)`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L127) `== key` — only works because of the sort from [](#boot-jump-label-init){.secref}: every
...
[`jump_label_type(entry)`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L455) computes `enabled ^ branch` — the exact
...
[`JUMP_LABEL_NOP`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L186) or a [`JUMP_LABEL_JMP`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L187).

Before acting on that answer, [`jump_label_can_update()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L465) filters out two
...
which is exactly what the [`jump_entry_is_init`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L158) flag from [](#boot-jump-label-init){.secref} was recorded to
detect), and one that [`kernel_text_address()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/extable.c#L94) does not even recognize as live
...
[`arch_jump_label_transform_queue()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/jump_label.c#L123) builds up a batch, and
[`arch_jump_label_transform_apply()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/jump_label.c#L143) executes it. Splitting them apart is
